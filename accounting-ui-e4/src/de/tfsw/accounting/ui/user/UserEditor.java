...
public class UserEditor extends AbstractFormBasedEditor {
	...
	private void createBasicInfoSection(Composite parent) {
		...
		createText(basicSectionClient, currentUser.getName(), currentUser, User.FIELD_NAME);
		
		toolkit.createLabel(basicSectionClient, messages.labelTaxId);
		createText(basicSectionClient, currentUser.getTaxNumber(), currentUser, User.FIELD_TAX_NUMBER);
...
//		LOG.debug("Total number of available currencies: " + index); //$NON-NLS-1$
		...
		Text description = toolkit.createText(basicSectionClient, currentUser.getDescription(), SWT.MULTI | SWT.BORDER);
